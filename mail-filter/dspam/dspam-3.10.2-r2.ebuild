...
PATCHES=(
	"${FILESDIR}"/001_dspam-webfrontend-config-debian.diff
	"${FILESDIR}"/002_update-dspam.conf.diff
	"${FILESDIR}"/003_dspam-default.prefs-in_etc.diff
	"${FILESDIR}"/005_dspam-notify.diff
	"${FILESDIR}"/006_default-daemon-port.diff
	"${FILESDIR}"/007_process_quarantine_if_spanish.diff
	"${FILESDIR}"/008_fix_exim_integration_doc.diff
	"${FILESDIR}"/009_fix_recipient_corruption_when_releasing_message_from_quarantine.diff
	"${FILESDIR}"/011_define_WCONTINUED_and_WIFCONTINUED_if_not_defined.diff
	"${FILESDIR}"/012_dont_read_past_end_of_css_files.diff
)
